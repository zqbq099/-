...
export const getGeminiKey = () => {
  return localStorage.getItem('mystique_gemini_key') || process.env.GEMINI_API_KEY;
};
...
export async function* streamMessage(message: string, history: ChatMessage[] = [], customInstructions?: string) {
  try {
    ...
    if (!apiKey) {
      throw new Error("لم يتم العثور على مفتاح Gemini API. يرجى إضافته من الإعدادات.");
    }
...
- **إغلاق الثغرات (Security First):** يمنع منعاً باتاً استخدام localStorage، sessionStorage، indexedDB أو التعامل مع window.parent (باستثناء postMessage). التطبيق يعمل في بيئة معزولة (Sandbox) واستخدام هذه التقنيات سيؤدي إلى انهيار التطبيق.
- **حفظ البيانات (Data Persistence):** لا تحفظ البيانات محلياً. إذا كان التطبيق يحتاج لحفظ بيانات، استخدم جسر البيانات السحابي الخاص بنا!
  - لحفظ بيانات في السحابة: 
    window.parent.postMessage({ type: 'MYSTIQUE_SAVE', collection: 'mystorage', data: { tasks: ['مهمة 1'] } }, '*');
  - للاستماع للتحديثات الحية (Real-time) واسترجاع البيانات:
    window.parent.postMessage({ type: 'MYSTIQUE_LISTEN', collection: 'mystorage' }, '*');
  - لاستقبال التحديثات في كودك:
    window.addEventListener('message', (e) => { if(e.data && e.data.type === 'MYSTIQUE_UPDATE') { const allData = e.data.data; /* تحديث الواجهة */ } });
...
- **التصميم المتجاوب أولاً (Mobile-First Responsive Design):** يجب أن تكون جميع التطبيقات التي تولدها متجاوبة بالكامل وتعمل بشكل مثالي على الهواتف المحمولة. استخدم Tailwind CSS بشكل مكثف لضمان تناسق الخطوط، المسافات، وأحجام العناصر على مختلف الشاشات.
- **جماليات واجهة المستخدم (Modern UI Aesthetics):** اجعل التطبيقات تبدو حديثة واحترافية. استخدم تدرجات لونية (Gradients)، وظلال ناعمة (Soft Shadows)، وزوايا مستديرة (Rounded Corners). اهتم بتجربة المستخدم (UX) من خلال إضافة تأثيرات عند النقر (Hover/Active states).
